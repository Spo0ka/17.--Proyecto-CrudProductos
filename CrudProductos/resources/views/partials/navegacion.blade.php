    ...
    <div class="container d-flex justify-content-start bg-white">
        ...
        <div id="navbarNav" class="navbar-collapse collapse">
            <ul class="navbar-nav ml-auto">
                <li class="nav-item">
                    <a class="nav-link" href="{{route('ProductosIndex')}}">Productos</a>
                </li>
                <li class="nav-item">
                    <form action="{{route('LogoutStore')}}" method="POST">
                    @csrf
                    <button type="submit" class="btn btn-primary" method="post">Cerrar Sesion </button>
                    </form>
                </li>
                <li class="nav-item">
                  <span class="nav-link"> Usuario: {{auth()->user()->username}}</span>
                </li>
            </ul>
        </div>
